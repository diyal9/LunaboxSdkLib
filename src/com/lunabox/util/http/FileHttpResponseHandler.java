...
public class FileHttpResponseHandler extends AsyncHttpResponseHandler
{
	...
	protected void sendResponseMessage(HttpResponse response)
	{
		...
		try
		{
			...
			LogWriter.print("totalSize: " + totalSize);

			if (file.exists() && totalSize == file.length())
			{
				LogWriter.print("Output file already exists. Skipping download.");
			...
			} else if (tempFile.exists())
			{
				...
				// +
				// "-");
				LogWriter.print("yahooo: "
						+ response.getEntity().getContentLength());
				previousFileSize = tempFile.length();

				LogWriter.print("File is not complete, download now.");
				LogWriter.print("File length:" + tempFile.length()
						+ " totalSize:" + totalSize);
			...
			LogWriter.print("Download completed successfully.");
			result = bytesCopied;
	...
	public int copy(InputStream input, RandomAccessFile out) throws IOException
	{
		...
		LogWriter.print("length" + out.length());
		int count = 0, n = 0;
